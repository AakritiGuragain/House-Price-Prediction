...
import os
import numpy as np # Needed for numpy operations (e.g., in utils)

# Import logic modules
# Note: PROCESSED_DATA_PATH is imported from utils and aliased in clustering
...
from src.chatbot import predict_price, load_artifacts 
...
if page == "1. Introduction & Setup":
    ...
    to deployment of an **Intelligent NLP Prediction Chatbot**.
    """)
...
# --- PHASE 3: K-Means Clustering (Step 5) ---
    ...
    try:
        ...
        st.stop() # <-- CORRECTED: Stops Streamlit execution gracefully
    ...
    st.pyplot(fig_elbow)
    st.info("The optimal 'K' is chosen where the drop in WCSS (inertia) starts to flatten (the 'elbow').")
...
# --- PHASE 4: ML Prediction Models & Chatbot (Step 6-9) ---
    ...
    try:
        ...
        if isinstance(model, str):
            st.error("Chatbot cannot load model artifacts.")
        st.stop() # <-- CORRECTED: Stops Streamlit execution gracefully

    # --- 4.2 Prediction Chatbot (Step 8 & 9) ---
    st.subheader("4.2 Intelligent Price Prediction Chatbot 🤖")
    st.markdown("""
    **System Instructions:** Ask for a prediction using natural language. The system extracts features, infers the market segment (Cluster), and uses the trained Random Forest model to provide a forecast.
    
    *Example Query: `I need a 4 bhk house in Kathmandu with 2 bathrooms and around 6 aana land area.`*
    """)
    
    user_query = st.text_input("Enter your house requirement:", 
                               value="I need a 4 bhk house in Kathmandu with 2 bathrooms and around 6 aana land area.", 
                               key="prediction_query")

    if user_query:
        prediction, input_df, cluster_map = predict_price(user_query)
        ...
        with col2:
            ...
            inferred_profile = cluster_map[cluster_map['Cluster'] == inferred_cluster_id].T
